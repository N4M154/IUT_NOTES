...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        self.costFn = lambda x: 1  # Cost function for moving to a corner is always 1
    ...
    def isGoalState(self, state):
        ...
        # Goal is achieved when all corners are visited
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
                ...
                nextx, nexty = int(x + dx), int(y + dy)
                if not self.walls[nextx][nexty]:
                    # Check if the new position is a corner and update cornersVisited
                    nextPosition = (nextx, nexty)
                    updatedCorners = list(cornersVisited)
                    if nextPosition in self.corners:
                        cornerIndex = self.corners.index(nextPosition)
                        updatedCorners[cornerIndex] = True
                    # Add successor with updated corners visited status
                    successors.append(((nextPosition, tuple(updatedCorners)), action, 1))
        self._expanded += 1 # DO NOT CHANGE
        return successors
...
def cornersHeuristic(state, problem):
    ...
    # Compute the Manhattan distance to the farthest unvisited corner
    distances_to_corners = [abs(current_position[0] - corner[0]) + abs(current_position[1] - corner[1])
