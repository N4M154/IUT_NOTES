...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not fringe.isEmpty():
        currentState, steps, existedCost = fringe.pop()
        ...
        for state, action, cost in problem.getSuccessors(currentState):
            if state not in visited or visited[state] > existedCost + cost:
                fringe.push((state, steps + [ action ], existedCost + cost), existedCost + cost + heuristic(state, problem))
