...
find_primes:
    ...
    ; Skip if number is 0, 1, or even (except 2)
    cmp al, 1
    jle not_prime
    cmp al, 2
    je check_two
    test al, 1           ; Test least significant bit
    jz not_prime         ; If even, skip prime check
    
    ; Check if number is prime (3,5,7)
    cmp al, 3
    je is_prime
    cmp al, 5
    je is_prime
...
check_two:
    ; 2 is prime but even, so skip it
    jmp not_prime
...
inner_loop:
    ...
    mov dl, [si+1]
    cmp al, dl
    jle no_swap          ; If already in order, skip swap
